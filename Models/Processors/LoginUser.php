...
class LoginUser
{
    ...
    public function processLogin(array $POSTdata): void
    {
        ...
        if (mb_strlen($POSTdata['name']) === 0) {
            $errors[] = AccessDeniedException::REASON_LOGIN_NO_NAME;
        }
        if (mb_strlen($POSTdata['pass']) === 0) {
            $errors[] = AccessDeniedException::REASON_LOGIN_NO_PASSWORD;
        }
    ...
    private function authenticate(string $username, string $password): array
    {
        if (str_contains($username, '@')) {
            ...
            $userData = Db::fetchQuery('SELECT * FROM '.User::TABLE_NAME.' WHERE '.
                                       LoggedUser::COLUMN_DICTIONARY['name'].' = ? LIMIT 1', array($username), false);
        }
        if ($userData === false) {
            throw new AccessDeniedException(AccessDeniedException::REASON_LOGIN_NONEXISTANT_USER, null, null);
        }
        if (!password_verify($password, $userData[LoggedUser::COLUMN_DICTIONARY['hash']])) {
            throw new AccessDeniedException(AccessDeniedException::REASON_LOGIN_WRONG_PASSWORD, null, null);
        } else {
            return $userData;
        }
    }
     ...
     * @return array|boolean Data o uživateli uložená v databázi, k jehož účtu se lze pomocí daného kódu přihlásit nebo
     *     FALSE, pokud je kód neplatný
     * @throws DatabaseException Pokud se při práci s databází vyskytne chyba
     * @throws AccessDeniedException Pokud není kód platný
     */
    private function verifyCode(string $code): array
    {
        $userData = Db::fetchQuery('SELECT * FROM '.User::TABLE_NAME.' WHERE '.LoggedUser::COLUMN_DICTIONARY['id'].
                                   ' = (SELECT uzivatele_id FROM sezeni WHERE kod_cookie = ? AND expirace > ? LIMIT 1);',
            array(md5($code), time()), false);
        if ($userData === false) {
            throw new AccessDeniedException(AccessDeniedException::REASON_LOGIN_INVALID_COOKIE_CODE, null, null);
        } else {
            return $userData;
        }
    }
    ...
    private function login(array $userData): void
    {
        ...
        $user->initialize($userData[LoggedUser::COLUMN_DICTIONARY['name']],
            $userData[LoggedUser::COLUMN_DICTIONARY['email']], new Datetime(),
            $userData[LoggedUser::COLUMN_DICTIONARY['addedPictures']],
        ...
        Db::executeQuery('UPDATE '.User::TABLE_NAME.' SET '.LoggedUser::COLUMN_DICTIONARY['lastLogin'].
                         ' = NOW() WHERE '.LoggedUser::COLUMN_DICTIONARY['id'].' = ?',
            array($userData[LoggedUser::COLUMN_DICTIONARY['id']]));
    }
