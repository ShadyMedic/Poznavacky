...
class RegisterUser
{
    ...
    private function validateData(string $name, string $pass, string $repass, $email): bool
    {
        ...
        if (mb_strlen($name) === 0) {
            $errors[] = AccessDeniedException::REASON_REGISTER_NO_NAME;
        }
        if (mb_strlen($pass) === 0) {
            $errors[] = AccessDeniedException::REASON_REGISTER_NO_PASSWORD;
        }
        if (mb_strlen($repass) === 0) {
            $errors[] = AccessDeniedException::REASON_REGISTER_NO_REPEATED_PASSWORD;
        }
        ...
        try {
            ...
            if ($e->getMessage() === 'long') {
                ...
                if ($e->getMessage() === 'short') {
                    $errors[] = AccessDeniedException::REASON_REGISTER_NAME_TOO_SHORT;
                }
            }
        }
        try {
            ...
            if ($e->getMessage() === 'long') {
                ...
                if ($e->getMessage() === 'short') {
                    $errors[] = AccessDeniedException::REASON_REGISTER_PASSWORD_TOO_SHORT;
                }
            }
        }
        ...
        try {
            ...
            $validator->checkCharacters($pass, DataValidator::USER_PASSWORD_ALLOWED_CHARS,
    ...
    private function register(string $name, string $password, ?string $email): bool
    {
        ...
        $user->initialize($name, $email, new DateTime(), null, null, self::DEFAULT_KARMA, self::DEFAULT_STATUS,
            $password);
        $user->save();
