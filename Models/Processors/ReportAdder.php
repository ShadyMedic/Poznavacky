...
class ReportAdder
{
    ...
    public function processFormData(array $POSTdata): bool
    {
        ...
        {
            (new Logger(true))->warning('Uživatel s ID {userId} se pokusil odeslat hlášení obrázku s URL {picUrl} v poznávačce s ID {groupId}, avšak nespecifikoval platný důvod', array('userId' => UserManager::getId(), 'picUrl' => $url, 'groupId' => $this->group->getId()));
        ...
        {
            (new Logger(true))->warning('Uživatel s ID {userId} se pokusil odeslat hlášení obrázku s URL {picUrl} v poznávačce s ID {groupId} z důvodu {reason}, avšak nevyplnil správně dodatečné informace', array('userId' => UserManager::getId(), 'picUrl' => $url, 'groupId' => $this->group->getId(), 'reason' => $reason));
        ...
        if ($dbResult === false)
        {
            (new Logger(true))->warning('Uživatel s ID {userId} se pokusil odeslat hlášení obrázku s URL {picUrl} v poznávačce s ID {groupId} z důvodu {reason}, avšak obrázek nemohl být v databázi nalezen', array('userId' => UserManager::getId(), 'picUrl' => $url, 'groupId' => $this->group->getId(), 'reason' => $reason));
        ...
        $natural->initialize($dbResult[Natural::COLUMN_DICTIONARY['name']], null, $dbResult[Natural::COLUMN_DICTIONARY['picturesCount']]);
        ...
        try
        {
            ...
            (new Logger(true))->info('Uživatel s ID {userId} odeslal hlásení obrázku s URL {picUrl} v poznávačce s ID {groupId} z důvodu {reason} (hlášení tohoto typu již existuje a tak byl pouze zvýšen počet nahlašovatelů)', array('userId' => UserManager::getId(), 'picUrl' => $url, 'groupId' => $this->group->getId(), 'reason' => $reason));
        }
        catch (BadMethodCallException $e)
        {
            ...
            $result = $report->save();    //Uložení hlášení do databáze
            (new Logger(true))->info('Uživatel s ID {userId} odeslal hlásení obrázku s URL {picUrl} v poznávačce s ID {groupId} z důvodu {reason}', array('userId' => UserManager::getId(), 'picUrl' => $url, 'groupId' => $this->group->getId(), 'reason' => $reason));
        }
        return $result;
    }
}
