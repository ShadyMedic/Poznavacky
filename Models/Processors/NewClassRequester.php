...
class NewClassRequester
{
    ...
    public function processFormData(array $POSTdata): bool
    {
        ...
        if ($this->validate($email, $name, $antispam)) {
            ...
            $composer->composeMail(EmailComposer::EMAIL_TYPE_NEW_CLASS_REQUEST, array(
                'username' => UserManager::getName(),
                'websiteAddress' => $_SERVER['SERVER_NAME'],
                'name' => htmlspecialchars($name),
                'message' => nl2br(htmlspecialchars($text)),
                'email' => htmlspecialchars($email)
            ));
            
            $sender = new EmailSender();
            return $sender->sendMail(self::WEBMASTER_EMAIL, 'Žádost o založení nové třídy od '.UserManager::getName(),
                $composer->getMail());
        }
        return false;
    }
    ...
    private function validate($email, string $name, $antispam): bool
    {
        ...
        if (mb_strlen($email) === 0 && empty(UserManager::getEmail())) {
            (new Logger(true))->notice('Pokus o odeslání formuláře pro založení nové třídy uživatelem s ID {userId} z IP adresy {ip} selhal, protože nebyl vyplněn kontaktní e-mail',
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($email)) {
            (new Logger(true))->notice('Pokus o odeslání formuláře pro založení nové třídy uživatelem s ID {userId} z IP adresy {ip} selhal, protože zadaný kontaktní e-mail ({email}) neměl platný formát',
