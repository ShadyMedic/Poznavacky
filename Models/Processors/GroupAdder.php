...
/**
 * Třída starající se o zpracování dat odeslaných z formuláře pro přidání poznávačky do třídy a případně vytvářející
 * novou poznávačku
 * @author Jan Štěch
 */
class GroupAdder
{
    ...
    public function checkData(string $groupName): bool
    {
        ...
        try {
            ...
            throw new AccessDeniedException(AccessDeniedException::REASON_MANAGEMENT_NEW_GROUP_DUPLICATE_NAME, null,
                $e);
        }
        ...
        try {
            $validator->checkLength($groupName, DataValidator::GROUP_NAME_MIN_LENGTH,
            ...
            if ($e->getMessage() === 'long') {
                throw new AccessDeniedException(AccessDeniedException::REASON_MANAGEMENT_NEW_GROUP_NAME_TOO_LONG, null,
                    $e);
            } else {
                if ($e->getMessage() === 'short') {
                    throw new AccessDeniedException(AccessDeniedException::REASON_MANAGEMENT_NEW_GROUP_NAME_TOO_SHORT,
                        null, $e);
                }
            }
        } catch (InvalidArgumentException $e) {
            ...
            throw new AccessDeniedException(AccessDeniedException::REASON_MANAGEMENT_NEW_GROUP_NAME_INVALID_CHARACTERS,
                null, $e);
        }
