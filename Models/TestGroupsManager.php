...
class TestGroupsManager
{
    ...
    {
        Db::connect();
        $classes = Db::fetchQuery('SELECT nazev,skupiny FROM `tridy` WHERE status = "public" OR tridy_id IN (SELECT tridy_id FROM clenstvi WHERE uzivatele_id = ?);', array(UserManager::getId()), true);
        if (!$classes)
        {
