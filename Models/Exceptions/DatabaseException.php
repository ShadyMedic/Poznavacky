...
class DatabaseException extends Exception
{
     ...
     * @return string Řetězec ve formátu "<zpráva> - error code: <kód>" nebo "Error code: <kód>" nebo "<zpráva>" nebo
     *     prázdný řetězec
     */
    public function getSafeInfo(bool $message, bool $code): string
    {
        ...
        if ($message && $code) {
            $result .= ' - Error code: '.$this->code;
        } else {
            if ($code) {
                $result .= 'An error occured. Error code: '.$this->code;
            }
        }
        return $result;
    }
    ...
    /**
     * Metoda zajišťující, že při neočekávaném výskytu výjimky nebudou vypsány citlivé informace z $previous
     * DBOexception
     * {@inheritDoc}
