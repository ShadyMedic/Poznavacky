...
class Administration
{
    ...
    {
        if ($includeLogged) {
            $dbResult = Db::fetchQuery('SELECT '.User::COLUMN_DICTIONARY['id'].','.User::COLUMN_DICTIONARY['name'].','.
                                       User::COLUMN_DICTIONARY['email'].','.User::COLUMN_DICTIONARY['lastLogin'].','.
                                       User::COLUMN_DICTIONARY['addedPictures'].','.
                                       User::COLUMN_DICTIONARY['guessedPictures'].','.
                                       User::COLUMN_DICTIONARY['guessedPictures'].','.User::COLUMN_DICTIONARY['karma'].
                                       ','.User::COLUMN_DICTIONARY['status'].' FROM '.User::TABLE_NAME, array(), true);
        } else {
            $dbResult = Db::fetchQuery('SELECT '.User::COLUMN_DICTIONARY['id'].','.User::COLUMN_DICTIONARY['name'].','.
                                       User::COLUMN_DICTIONARY['email'].','.User::COLUMN_DICTIONARY['lastLogin'].','.
                                       User::COLUMN_DICTIONARY['addedPictures'].','.
                                       ...
                                       User::COLUMN_DICTIONARY['id'].' != ?', array(UserManager::getId()), true);
        }
        $users = array();
        foreach ($dbResult as $dbRow) {
            $lastLogin = new DateTime($dbRow[User::COLUMN_DICTIONARY['lastLogin']]);
            $user = new User(false, $dbRow[User::COLUMN_DICTIONARY['id']]);
            $user->initialize($dbRow[User::COLUMN_DICTIONARY['name']], $dbRow[User::COLUMN_DICTIONARY['email']],
                $lastLogin, $dbRow[User::COLUMN_DICTIONARY['addedPictures']],
                $dbRow[User::COLUMN_DICTIONARY['guessedPictures']], $dbRow[User::COLUMN_DICTIONARY['karma']],
    ...
    {
        $dbResult = Db::fetchQuery('SELECT '.ClassObject::COLUMN_DICTIONARY['id'].','.
    ...
    public function getAdminReports(): array
    {
        ...
        $result = Db::fetchQuery('
            ...
            '.Picture::TABLE_NAME.'.'.Picture::COLUMN_DICTIONARY['id'].' AS "obrazky_id", '.Picture::TABLE_NAME.'.'.
                                 Picture::COLUMN_DICTIONARY['url'].' AS "obrazky_zdroj", '.Picture::TABLE_NAME.'.'.
    ...
    public function getUserNameChangeRequests(): array
    {
        ...
        '.User::TABLE_NAME.'.'.User::COLUMN_DICTIONARY['id'].', '.User::TABLE_NAME.'.'.User::COLUMN_DICTIONARY['name'].
                                 ...
                                 User::COLUMN_DICTIONARY['status'].',
        '.UserNameChangeRequest::TABLE_NAME.'.'.UserNameChangeRequest::COLUMN_DICTIONARY['id'].', '.
                                 UserNameChangeRequest::TABLE_NAME.'.'.
                                 UserNameChangeRequest::COLUMN_DICTIONARY['newName'].', '.
                                 UserNameChangeRequest::TABLE_NAME.'.'.
                                 UserNameChangeRequest::COLUMN_DICTIONARY['requestedAt'].'
        FROM '.UserNameChangeRequest::TABLE_NAME.'
        JOIN '.User::TABLE_NAME.' ON '.UserNameChangeRequest::TABLE_NAME.'.'.
        ...
        foreach ($result as $requestInfo) {
            ...
            $user->initialize($requestInfo[User::COLUMN_DICTIONARY['name']],
                $requestInfo[User::COLUMN_DICTIONARY['email']],
                new DateTime($requestInfo[User::COLUMN_DICTIONARY['lastLogin']]),
                $requestInfo[User::COLUMN_DICTIONARY['addedPictures']],
                $requestInfo[User::COLUMN_DICTIONARY['guessedPictures']],
                $requestInfo[User::COLUMN_DICTIONARY['karma']], $requestInfo[User::COLUMN_DICTIONARY['status']]);
            $request = new UserNameChangeRequest(false, $requestInfo[UserNameChangeRequest::COLUMN_DICTIONARY['id']]);
            $request->initialize($user, $requestInfo[UserNameChangeRequest::COLUMN_DICTIONARY['newName']],
                new DateTime($requestInfo[UserNameChangeRequest::COLUMN_DICTIONARY['requestedAt']]));
            $requests[] = $request;
        }
        return $requests;
    }
    ...
    public function getClassNameChangeRequests(): array
    {
        ...
        '.ClassNameChangeRequest::TABLE_NAME.'.'.ClassNameChangeRequest::COLUMN_DICTIONARY['id'].', '.
                                 ...
                                 ClassNameChangeRequest::TABLE_NAME.'.'.
                                 ClassNameChangeRequest::COLUMN_DICTIONARY['requestedAt'].'
        FROM '.ClassNameChangeRequest::TABLE_NAME.'
        JOIN '.ClassObject::TABLE_NAME.' ON '.ClassNameChangeRequest::TABLE_NAME.'.'.
                                 ClassNameChangeRequest::COLUMN_DICTIONARY['subject'].' = '.ClassObject::TABLE_NAME.'.'.
        ...
        foreach ($result as $requestInfo) {
            ...
            $admin->initialize($requestInfo[User::COLUMN_DICTIONARY['name']],
                $requestInfo[User::COLUMN_DICTIONARY['email']],
                new DateTime($requestInfo[User::COLUMN_DICTIONARY['lastLogin']]),
                $requestInfo[User::COLUMN_DICTIONARY['addedPictures']],
                $requestInfo[User::COLUMN_DICTIONARY['guessedPictures']],
                $requestInfo[User::COLUMN_DICTIONARY['karma']], $requestInfo['u_status']);
     ...
     * @throws AccessDeniedException Pokud jsou zadané hodnoty neplatné nebo přihlášený uživatel není systémový
     *     administrátor
     * @throws DatabaseException
     */
     ...
     * @throws AccessDeniedException Pokud není přihlášený uživatel systémovým administrátorem nebo pokud odstraňovaný
     *     uživatel spravuje nějakou třídu
     * @throws DatabaseException
     */
     ...
     * @throws AccessDeniedException Pokud není přihlášený uživatel systímovým administrátorem nebo jsou zadaná data
     *     neplatná
     * @throws DatabaseException
     */
     ...
     * @param string $changedIdentifier Údaj o tom, zda je druhý argument této metody ID nového správce třídy, nebo
     *     jeho jméno
     * @return User Objekt uživatele reprezentující právě nastaveného správce třídy
     * @throws DatabaseException
     * @throws AccessDeniedException Pokud není některý z údajů platný (například pokud uživatel s daným ID nebo jménem
     *     neexistuje)
     * @throws \Exception  Pokud se nepodaří vytvořit objekt DateTime
     */
        ...
        switch ($changedIdentifier) {
            case 'id':
                $result = Db::fetchQuery('SELECT '.User::COLUMN_DICTIONARY['id'].', '.User::COLUMN_DICTIONARY['name'].
                                         ', '.User::COLUMN_DICTIONARY['email'].', '.
                                         User::COLUMN_DICTIONARY['lastLogin'].', '.
                                         User::COLUMN_DICTIONARY['addedPictures'].', '.
                                         User::COLUMN_DICTIONARY['guessedPictures'].', '.
                                         User::COLUMN_DICTIONARY['karma'].', '.User::COLUMN_DICTIONARY['status'].
                                         ' FROM '.User::TABLE_NAME.' WHERE '.User::COLUMN_DICTIONARY['id'].' = ?',
                    array($newAdminIdentifier), false);
                break;
            case 'name':
                $result = Db::fetchQuery('SELECT '.User::COLUMN_DICTIONARY['id'].', '.User::COLUMN_DICTIONARY['name'].
                                         ', '.User::COLUMN_DICTIONARY['email'].', '.
                                         ...
                                         User::COLUMN_DICTIONARY['guessedPictures'].', '.
                                         User::COLUMN_DICTIONARY['karma'].', '.User::COLUMN_DICTIONARY['status'].
                                         ' FROM '.User::TABLE_NAME.' WHERE '.User::COLUMN_DICTIONARY['name'].' = ?',
                    array($newAdminIdentifier), false);
                break;
            default:
     ...
     * @param bool $classNameChange TRUE, pokud se žádost týká změny jména třídy, FALSE, pokud změny uživatelského
     *     jména
     * @param bool $approved TRUE, pokud byla žádost schválena, FALSE, pokud zamítnuta
     * @param string $reason V případě zamítnutí žádosti důvod jejího zamítnutí - je odesláno e-mailem uživateli; při
     *     schválení žádosti nepovinné
    ...
    {
        $request = ($classNameChange) ? new ClassNameChangeRequest(false, $requestId) :
            new UserNameChangeRequest(false, $requestId);
        if ($approved) {
     ...
     * @param string $rawMessage Obsah hlavního těla e-mailu (může být zformátován pomocí HTML)
     * @param string $rawFooter Obsah patičky e-mailu (může být zformátován pomocí HTML), může být jako jediný parametr
     *     prázdný řetězec
     * @param string $sender Jméno odesílatele e-mailu, které bude zobrazeno jako odesílatel e-mailové zprávy
     * @param string $fromAddress E-mailová adresa pro odpověď (bude to tak trochu vypadat, jako kdyby e-mail přišel z
     *     této adresy)
     ...
     */
    public function sendEmail(string $addressee, string $subject, string $rawMessage, string $rawFooter, string $sender,
                              string $fromAddress): bool
    {
        //Kontrola platnosti e-mailů
        if (!filter_var($addressee, FILTER_VALIDATE_EMAIL)) {
            throw new AccessDeniedException(AccessDeniedException::REASON_SEND_EMAIL_INVALID_ADDRESSEE_ADDRESS, null,
                null);
        }
    ...
    public function executeSqlQueries(string $queries): string
    {
        ...
        if (empty($cnt) && !empty($queries)) {
            $cnt++;
        }     //Pokud není přítomen žádný středník (a byl odeslán nějaký text), provedeme ten jeden jediný, co nekončí středníkem
        for ($i = 0; $i < $cnt; $i++) {
            echo '<p>';
