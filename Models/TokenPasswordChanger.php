...
class TokenPasswordChanger
{
    ...
    public function verifyToken(): void
    {
        ...
        {
            (new Logger(true))->warning('Uživatel s přístupem na stránku pro obnovení hesla se pokusil změnit heslo z IP adresy {ip}, avšak jeho kód (hash {hash}) je neplatný', array('ip' => $_SERVER['REMOTE_ADDR'], 'hash' => md5($this->token)));
    ...
    public function devalueToken(): void
    {
        ...
        (new Logger(true))->info('Použitý kód pro obnovu hesla (hash {hash}) byl odstraněn z databáze', array('hash' => md5($this->token)));
    }
    ...
    function checkPasswords(): void
    {
        ...
        catch (RangeException $e)
        {
            ...
            {
                (new Logger(true))->notice('Pokus o změnu hesla pomocí kódu pro obnovení (hash {hash}) k uživatelskému účtu s ID {userId} z IP adresy {ip} selhal kvůli nepřijatelné délce hesla', array('hash' => md5($this->token), 'userId' => $this->userId, 'ip' => $_SERVER['REMOTE_ADDR']));
        ...
        {
            (new Logger(true))->notice('Pokus o změnu hesla pomocí kódu pro obnovení (hash {hash}) k uživatelskému účtu s ID {id} z IP adresy {ip} selhal kvůli přitomnosti nepovolených znaků', array('hash' => md5($this->token), 'userId' => $this->userId, 'ip' => $_SERVER['REMOTE_ADDR']));
            throw new AccessDeniedException(AccessDeniedException::REASON_REGISTER_PASSWORD_INVALID_CHARACTERS, null, null);
        }
        
        if ($this->pass !== $this->repass)
        {
            (new Logger(true))->notice('Pokus o změnu hesla pomocí kódu pro obnovení (hash {hash}) z IP adresy {ip} selhal kvůli neshodě mezi zadanými hesly', array('hash' => md5($this->token), 'ip' => $_SERVER['REMOTE_ADDR']));
    ...
    function changePassword(): bool
    {
        ...
        (new Logger(true))->info('Pomocí kódu pro obnovení hesla (hash {hash}) bylo z IP adresy {ip} změněno heslo k uživatelskému účtu s ID {userId}', array('ip' => $_SERVER['REMOTE_ADDR'], 'hash' => md5($this->token), 'userId' => $this->userId));
        return true;
    }
}
