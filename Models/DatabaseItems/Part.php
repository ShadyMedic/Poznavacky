...
class Part extends Folder
{
     ...
     * @param int|undefined|null $naturalsCount Počet přírodnin v této části poznávačky (při vyplnění parametru
     *     $naturals je ignorováno a je použita délka poskytnutého pole)
     * @param int|undefined|null $picturesCount Počet obrázků v této části poznávačky
     * {@inheritDoc}
     * @see DatabaseItem::initialize()
     */
    public function initialize($name = null, $url = null, $group = null, $naturals = null, $naturalsCount = null,
                               $picturesCount = null): void
    {
        ...
        if ($naturals === null) {
            ...
            if ($naturalsCount === null) {
                $naturalsCount = $this->naturalsCount;
            }
        } else {
    ...
    public function loadNaturals(): void
    {
        ...
        $result = Db::fetchQuery('SELECT '.Natural::COLUMN_DICTIONARY['id'].','.Natural::COLUMN_DICTIONARY['name'].','.
                                 Natural::COLUMN_DICTIONARY['picturesCount'].' FROM '.Natural::TABLE_NAME.' WHERE '.
                                 Natural::COLUMN_DICTIONARY['id'].
                                 ' IN (SELECT prirodniny_id FROM prirodniny_casti WHERE casti_id = ?)',
            array($this->id), true);
        if ($result === false || count($result) === 0) {
            ...
            foreach ($result as $naturalData) {
                ...
                $natural->initialize($naturalData[Natural::COLUMN_DICTIONARY['name']], null,
                    $naturalData[Natural::COLUMN_DICTIONARY['picturesCount']]);
