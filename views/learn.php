...
<html>
	...
	<body onkeypress="keyPressed(event)">
        ...
        <header>
            <h1>Učit se</h1>
			<div><?= ?><!-- PHP - ZDE NAPSÁNO, KDO JE PŘIHLÁŠENÝ --></div>
        </header>
		<aside><?= ?><!-- PHP - ZDE NAVIGACE --></aside>
    		...
    		<fieldset>
    			<div class="prikaz">Vyberte si přírodninu, jejíž obrázky si chcete prohlížet. Na další nebo předchozí přírodninu můžete přejít rychle pomocí tlačítek.</div>
    		  	<select onchange="sel()" id="dropList" class="text dropList">
					...
					<script>naturalList.push('$name');</script> <?= ?><!-- Z PHP -->
					<option>$name</option> <?= ?><!-- Z PHP (nutno zabalit do cyklu) -->
    		...
    		<fieldset>
    			<table>
    				<tr>
    					...
    					<td>
    						<img id="image" class="img" src="images/imagePreview.png"><?= ?><!-- MOŽNÁ PHP ? -->
    					</td>
    					<td>
