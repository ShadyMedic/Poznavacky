...
class AnswerInvitationController extends AjaxController
{
     ...
     * @param array $parameters Parametry ke zpracování, prvním prvkem pole musí být URL třídy, na pozvánku do níž odpovídáme, druhým řetězec "accept" nebo "reject", podle odpovědi na pozvánku
    ...
    public function process(array $parameters): void
    {
        ...
        $invitationData = Db::fetchQuery('SELECT '.Invitation::COLUMN_DICTIONARY['id'].','.Invitation::COLUMN_DICTIONARY['class'].','.Invitation::COLUMN_DICTIONARY['expiration'].' FROM '.Invitation::TABLE_NAME.' WHERE '.Invitation::COLUMN_DICTIONARY['class'].' = (SELECT '.ClassObject::COLUMN_DICTIONARY['id'].' FROM '.ClassObject::TABLE_NAME.' WHERE '.ClassObject::COLUMN_DICTIONARY['url'].' = ?) AND '.Invitation::COLUMN_DICTIONARY['user'].' = ? AND '.Invitation::COLUMN_DICTIONARY['expiration'].' > NOW();', array($classUrl, UserManager::getId()));
        if (empty($invitationData))
        {
            //Pozvánka buďto neexistuje nebo vyexpirovala nebo není určena pro přihlášeného uživatele
            (new Logger(true))->notice('Uživatel s ID {userId} se pokusil odpovědět na pozvánku do třídy s URL {classUrl} z IP adresy {ip}, avšak daná pozvánka nebyla v databázi nalezena nebo nebyla určena pro tohoto uživatele', array('userId' => UserManager::getId(), 'classUrl' => $classUrl, 'ip' => $_SERVER['REMOTE_ADDR']));
            $response = new AjaxResponse(AjaxResponse::MESSAGE_TYPE_ERROR, 'Tato pozvánka neexistuje, není určená pro vás nebo již vypršela její platnost');
            echo $response->getResponseString();
            return;
        }
        ...
        if ($accepted)
        {
            ...
            $response = new AjaxResponse(AjaxResponse::MESSAGE_TYPE_SUCCESS, 'Pozvánka byla přijata. Nyní máte do třídy '.$invitation->getClass()->getName().' přístup.');
        }
        else
        {
