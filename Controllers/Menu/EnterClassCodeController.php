...
class EnterClassCodeController extends AjaxController
{
    ...
    public function process(array $parameters): void
    {
        ...
        if (!isset($_POST) || mb_strlen($_POST['code']) === 0) {
            ...
            (new Logger(true))->notice('Uživatel s ID {userId} odeslal formulář pro zadání vstupního kódu třídy z IP adresy {ip}, avšak nevyplnil do něj údaje',
        ...
        if (!$validator->validateClassCode($code)) {
            (new Logger(true))->notice('Uživatel s ID {userId} odeslal formulář pro zadání vstupního kódu třídy z IP adresy {ip}, avšak zadaný kód ({code}) nebyl ve správném formátu',
        ...
        if (count($classes) === 0) {
            ...
            (new Logger(true))->notice('Uživatel s ID {userId} odeslal vstupní kód třídy z IP adresy {ip}, avšak zadaný kód ({$code}) není možné použít k získání přístupu do žádné třídy',
                array('userId' => $userId, 'ip' => $_SERVER['REMOTE_ADDR'], 'code' => $code));
        ...
        (new Logger(true))->info('Uživatel s ID {userId} odeslal vstupní kód třídy ({code}) z IP adresy {ip} a získal přístup do tříd/y s ID {classesIds}',
            array(
                'userId' => $userId,
                'code' => $code,
