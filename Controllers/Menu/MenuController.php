...
class MenuController extends SynchronousController
{
    ...
    public function process(array $parameters): void
    {
        ...
        try {
            if (!$aChecker->checkClass()) {
                ...
                if (!$aChecker->checkGroup()) {
                    ...
                    (new Logger(true))->info('K uživateli s ID {userId} přistupujícímu do systému z IP adresy {ip} byl odeslán seznam poznávaček ve třídě s ID {classId}',
                        array(
                    ...
                    $lastVisitedFolderPath = $_SESSION['selection']['class']->getUrl().'/'.
                                             $_SESSION['selection']['group']->getUrl();
                    (new Logger(true))->info('K uživateli s ID {userId} přistupujícímu do systému z IP adresy {ip} byl odeslán seznam částí v poznávačce s ID {groupId} ve třídě s ID {classId}',
                        array(
                            'userId' => UserManager::getId(),
                            'ip' => $_SERVER['REMOTE_ADDR'],
            ...
            (new Logger(true))->notice('Uživatel s ID {userId} přistupující do systému z IP adresy {ip} odeslal požadavek na zobrazení obsahu třídy, poznávačky nebo části, která žádný obsah nemá',
                array('userId' => UserManager::getId(), 'ip' => $_SERVER['REMOTE_ADDR']));
            
            //Nahraď pohled s tabulkou pohledem pro obyčejnou hlášku
            for ($i = 0; $i < count(self::$views); $i++) {
                $view = self::$views[$i];
                if ($view === 'menuClassesTable' || $view === 'menuGroupsTable' || $view === 'menuPartsTable') {
        ...
        if (!$changelogManager->checkLatestChangelogRead()) {
            ...
            (new Logger(true))->info('Uživateli s ID {userId} byl zobrazen nejnovější changelog pro verzi {version}',
