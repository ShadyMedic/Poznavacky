        ...
        try {
            if ($requester->processFormData($_POST)) {
                ...
                $response = new AjaxResponse(AjaxResponse::MESSAGE_TYPE_SUCCESS,
                    'Tvá třída byla okamžitě vytvořena s dočasným jménem "Třída uživatele '.UserManager::getName().'". 
                    Zároveň byla vytvořena žádost o změnu názvu této třídy na "'.$_POST['className'].'". 
                    Administrátor by tuto žádost měl vyřídit v následujících hodinách (výjimečně dnech). 
                    V okamžiku schválení změny názvu obdržíš e-mail. Pro zobrazení tvé třídy aktualizuj tuto stránku.');
            } else {
                throw new Exception();
            }
            ...
            $response = new AjaxResponse(AjaxResponse::MESSAGE_TYPE_ERROR,
                'E-mail se nepodařilo odeslat. Zkus to prosím později, nebo nám dej vědět přes formulář "Nalezl*a jsi problém?" v zápatí stránky.',
                array('newCaptcha' => $this->renewCaptcha()));
        }
