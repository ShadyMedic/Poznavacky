...
abstract class SynchronousController implements ControllerInterface
{
    ...
    protected static array $pageHeader = array(
        'title' => 'Poznávačky',
        'keywords' => '',
        'description' => '',
        'cssFile' => array(),
    ...
    /**
     * Metoda kontrolující, zda ve složce ukládající kontrolery nebo získávače dat existuje daný soubor a v případě že
     * ano, vrací jeho celé jméno (obsahující jeho jmenný prostor) Kód této metody byl z části inspirován touto
     * odpovědi na StackOverflow https://stackoverflow.com/a/44315881/14011077
        ...
        foreach ($files as $value) {
            ...
            if (!is_dir($path) && $fileName == $value) //Soubor
            {
                ...
                if (is_dir($path)) //Složka
                {
                    ...
                    if ($resultFromSubdirectory) {
                        return $resultFromSubdirectory;
                    }
                }
            }
        }
