...
class RooterController extends SynchronousController
{
     ...
     * @throws AccessDeniedException Při pokusu zkontrolovat nějaký údaj o přihlášeném uživateli, pokud žádný uživatel
     *     přihlášen není
    ...
    public function process(array $parameters): void
    {
        ...
        $urlVariablesArr = array_diff($urlArguments,
            $controllersUrls); //Získá pole jednotlivých proměnných URL parametrů
        ...
        foreach (self::$views as $view) {
            ...
            if (empty($dataGetterName)) {
                continue;
            }
            $pathToDataGetter = $this->classExists($dataGetterName.self::DATA_GETTER_EXTENSION,
                self::DATA_GETTER_FOLDER);
        ...
        //Nastavení složek
        if (!$this->setFolders($selections,
            $urlVariablesValues)) //Předávání podle reference zajistí, že se spotřebované parametry odeberou
        {
            header('HTTP/1.0 404 Not Found');
            $this->controllerToCall = new Error404Controller();
            $checks = array();
     ...
     * @param array $selections Pole řetězců popisující význam a pořadí argumentů ve druhém argumentu, povolené hodnoty
     *     řetězců jsou 'class', 'group', 'part' a hodnoty konstant této třídy obsahující v názvu slovo 'SELECTOR'
     * @param array $urlVariablesValues Pole argumentů získaných z URL pro zpracování, pořadí prvků musí odpovídat
     *     hodnotám v prvním argumentu
     * @return bool TRUE, pokud se povedlo všechny složky nastavit, FALSE, pokud některá ze složek nebyla podle URL v
     *     databázi nalezena
    ...
    private function setFolders(array $selections, array &$urlVariablesValues): bool
    {
        ...
        for ($i = 0; $i < count($selections); $i++) {
            ...
            if ($selection[0] === self::CLEAR_SELECTION_PREFIX) {
                ...
                if ($selection === self::NON_SELECTION_VALUE) {
                    continue;
                } else {
                    if ($selection === self::IGNORE_SELECTION_VALUE) {
            ...
            switch ($selection) {
                case 'class':
                    $parentFolder = null;
                    $alreadySet = ($aChecker->checkClass() &&
                                   $_SESSION['selection']['class']->getUrl() === $currentUrl);
                    if (!$alreadySet) {
     ...
     * @param array $checks Pole řetězců popisujících kontroly, které se musejí provést, jejich význam je blíže popsán
     *     v souboru routes.ini
     * @return bool TRUE, pokud všechny kontroly proběhnou v pořádku, FALSE, pokud ne
     * @throws AccessDeniedException Pokud je kontrolováno cokoliv o přihlášeném uživateli, pokud žádný uživatel
     *     přihlášen není (toto se nemůže stát, pokud jsou kontroly správně seřazey - od nejmírnější po nejpřísnější)
     */
