...
{
	global $connection;
	$query = 'SELECT * FROM statistika ORDER BY statistika_id LIMIT 16';	//V tabulce statistika vždy musí zůstat zachována alespoň jedna řádka.
	...
	if ($numRows == 16)
	{
		...
		$dateFrom = $row['datum'];
		$timeFrom = $row['cas'];
		$idFrom = $row['statistika_id'];
		$sum = $row['pozadavky'];
		for ($i = 1; $i < 15; $i++)
		{
			$row = mysqli_fetch_array($result);
			$idTo = $row['statistika_id'];
			$sum += $row['pozadavky'];
		}
		//Poèítání prùmìru
		$sum /= 15;
		...
		$query = 'INSERT INTO pozadavky_za_15_minut VALUES(NULL, "'.$dateFrom.'", "'.$timeFrom.'", '.$sum.')';
		$result = mysqli_query($connection, $query);
		if (!$result){die('Error occured while executing '.$query);}
		
		$query = 'DELETE FROM statistika WHERE statistika_id BETWEEN '.$idFrom.' AND '.$idTo;
		$result = mysqli_query($connection, $query);
		if (!$result){die('Error occured while executing '.$query);}
...
$query = "SELECT statistika_id,pozadavky FROM statistika WHERE datum='$date' AND cas='$time'";
$result = mysqli_query($connection, $query);
...
if (mysqli_num_rows($result) == 0)
{
    ...
    $query = "SELECT statistika_id,datum,cas FROM statistika ORDER BY statistika_id DESC LIMIT 1";
    $result = mysqli_query($connection, $query);
    if (!$result){die("An error occured while working with mysql server. Error code: ".mysqli_errno($connection).". Please, conntact administrator.");}
    ...
    while ($date != $currentDate || $time != $currentTime)
    {
        ...
        date_add($dateObj, new DateInterval("PT1M"));    //Přidání jedné minuty;
